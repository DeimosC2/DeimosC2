...
/*
Code modified from https://github.com/Ne0nd0g/merlin/blob/master/pkg/modules/srdi/srdi.go
*/
...
func Parse(data string, args []string) ([]byte, error) {
	...
	options["pid"] = ""
	options["method"] = "self"
	options["function"] = "Module" //Function Name to call in the DLL
	options["clearHeader"] = "false"

	newArgs := DllOptions{}
	newArgs.Options = args
	mData, err := json.Marshal(newArgs)
	if err != nil {
		logging.ErrorLogger.Println(err.Error())
	}
	options["args"] = string(mData)
	...
	sc, errShellcode := dllToReflectiveShellcode(options["dll"], options["function"], options["args"])
	if errShellcode != nil {
...
//This code came mostly from an implementation by SpectorOps
...
func dllToReflectiveShellcode(dllPath string, functionName string, userDataStr string) ([]byte, error) {
...
func is64BitDLL(dllBytes []byte) bool {
	...
	if machine == machineIA64 || machine == machineAMD64 {
		logging.Logger.Println("64 bit")
		return true
	}
	logging.Logger.Println("32 bit")
	return false
}
