...
func ObfuscateSymbols(gopath string, n NameHasher, ctx build.Context) error {
	...
	if err := runRenames(gopath, renames, ctx); err != nil {
		return fmt.Errorf("top-level renaming: %s", err)
	}
	renames, err = methodRenames(gopath, n, ctx)
	if err != nil {
	...
	logging.Logger.Println("TOTAL SAVED RENAME COUNT IS:", saveCount)
	return nil
}

func runRenames(gopath string, renames []symbolRenameReq, ctx build.Context) error {
	for _, r := range renames {

		if !strings.Contains(r.OldName, "golang.org") { //save some processing power
			logging.Logger.Println("Changing ", r.OldName, "to: ", r.NewName)
			//TODO see if we can find a way to not use this or speed it up, it takes way to long to run and is the cause of our slow obfuscation
...
func topLevelRenames(gopath string, n NameHasher) ([]symbolRenameReq, error) {
	...
	err := filepath.Walk(srcDir, func(path string, info os.FileInfo, err error) error {
		...
		pkgPath = strings.Replace(pkgPath, string(filepath.Separator), "/", -1)
...
func methodRenames(gopath string, n NameHasher, ctx build.Context) ([]symbolRenameReq, error) {
	exclude, err := interfaceMethods(gopath, ctx)
	if err != nil {
		return nil, err
	}
...
func interfaceMethods(gopath string, ctx build.Context) (map[string]bool, error) {
	...
	for pkgName := range pkgs {
		...
		for _, fileName := range pkg.GoFiles {
			sourcePath := path.Join(pkg.Dir, fileName)
			set := token.NewFileSet()
