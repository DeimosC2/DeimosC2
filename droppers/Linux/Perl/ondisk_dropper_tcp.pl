...
if (index(my $proc = `cat /proc/cpuinfo | grep Intel`, 'Intel') != -1){
     $PROC = 'I';
}
elsif (index(my $proc = `cat /proc/cpuinfo | grep ARM`, 'ARM') != -1){
     $PROC = 'A';
} 
elsif (index(my $proc = `cat /proc/cpuinfo | grep MIPS`, 'MIPS')!= -1){
    $PROC = 'M';
}
